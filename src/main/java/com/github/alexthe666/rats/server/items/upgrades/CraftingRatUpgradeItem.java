...
public class CraftingRatUpgradeItem extends BaseRatUpgradeItem implements HoldsItemUpgrade, TickRatUpgrade {
	...
	public void renderHeldItem(EntityRendererProvider.Context context, TamedRat rat, RatModel<?> model, PoseStack stack, SubmitNodeCollector collector, int light, float ageInTicks) {
		...
		this.submitItem(new ItemStack(Items.STONE_AXE), rat, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, stack, collector, light);
		stack.popPose();
		stack.pushPose();
		...
		stack.translate(0.0F, -0.075F, -0.1F);
		stack.scale(0.65F, 0.65F, 0.65F);
		this.submitItem(new ItemStack(Items.STONE_PICKAXE), rat, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, stack, collector, light);
		stack.popPose();
	}
			...
			if (te instanceof RatCraftingTableBlockEntity table && !rat.level().isClientSide()) {
				...
				if (table.getCookTime() > 0) {
					...
					if (table.getRecipeUsed() != null) {
						// 26.1: Recipe.getResultItem(RegistryAccess) is gone; assemble the result from the table's matrix.
						ItemStack stack = table.getRecipeUsed().value().assemble(table.matrixWrapper.asCraftInput());
						if (stack.isEmpty()) {
							...
							// 26.1: ItemParticleOption takes an ItemStackTemplate now.
							...
							((ServerLevel) rat.level()).sendParticles(particle, rat.getX() + (double) (rat.getRandom().nextFloat() * rat.getBbWidth() * 2.0F) - (double) rat.getBbWidth(), rat.getY(), rat.getZ() + (double) (rat.getRandom().nextFloat() * rat.getBbWidth() * 2.0F) - (double) rat.getBbWidth(), 1, d0, d1, d2, 0);
							((ServerLevel) rat.level()).sendParticles(particle, rat.getX() + (double) (rat.getRandom().nextFloat() * rat.getBbWidth() * 2.0F) - (double) rat.getBbWidth(), rat.getY(), rat.getZ() + (double) (rat.getRandom().nextFloat() * rat.getBbWidth() * 2.0F) - (double) rat.getBbWidth(), 1, d0, d1, d2, 0);
						}
					}

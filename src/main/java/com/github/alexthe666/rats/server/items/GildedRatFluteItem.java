...
public class GildedRatFluteItem extends LoreTagItem {
	...
	public InteractionResult use(Level level, Player player, InteractionHand hand) {
		...
		// Upstream gated this on getTag() != null — accidentally always true pre-1.20.5 (damageable
		// items auto-wrote Damage:0 into NBT), but its mechanical CUSTOM_DATA translation is always
		// FALSE in the component era (durability lives in DataComponents.DAMAGE), so the flute
		// silently no-oped. Drop the gate; spawn the shot server-side only.

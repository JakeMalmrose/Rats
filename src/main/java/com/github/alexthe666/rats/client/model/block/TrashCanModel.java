...
public class TrashCanModel<T extends Entity> extends AdvancedEntityModel<T> {
	...
	public void animate(TrashCanBlockEntity trashCan) {
		...
		float openProgress = trashCan.prevLidProgress + (trashCan.lidProgress - trashCan.prevLidProgress) * Minecraft.getInstance().getDeltaTracker().getGameTimeDeltaPartialTick(true);

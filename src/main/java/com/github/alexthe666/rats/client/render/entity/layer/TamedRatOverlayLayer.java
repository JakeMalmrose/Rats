...
public class TamedRatOverlayLayer extends RenderLayer<LivingEntityRenderState, RatsEntityModelBridge<TamedRat>> {
	...
	public void submit(PoseStack stack, SubmitNodeCollector collector, int light, LivingEntityRenderState state, float netHeadYaw, float headPitch) {
		...
		if (rat.getRespawnCountdown() > 0 && RatConfig.ratAngelGlint) {
			this.submitOverlay(stack, collector, RatsRenderType.getWhiteGlint(), light, OverlayTexture.NO_OVERLAY, -1);
		} else {
			if (rat.hasCustomName() && rat.getCustomName().getString().equalsIgnoreCase("ultrakill")) {
				this.submitOverlay(stack, collector, RatsRenderType.getGreenGlint(), light, OverlayTexture.NO_OVERLAY, -1);
			} else {
				if (RatUpgradeUtils.hasUpgrade(rat, RatsItemRegistry.RAT_UPGRADE_GOD.get()) && RatConfig.ratGodGlint) {
					...
					this.submitOverlay(stack, collector, RenderTypes.entityCutout(this.renderer.getTextureLocation(state)), light, OverlayTexture.NO_OVERLAY, -1);
					this.submitOverlay(stack, collector, RenderTypes.entityGlint(), light, OverlayTexture.NO_OVERLAY, -1);
				}
				...
				if (rat.isDyed()) {
					...
					if (rat.getDyeColor() == 100) {
						...
						this.submitOverlay(stack, collector, type != null ? type : RatsRenderType.getRainbowGlint(), light, overlay, -1);
						if (!RatUpgradeUtils.hasUpgrade(rat, RatsItemRegistry.RAT_UPGRADE_UNDEAD.get())) {
							this.submitOverlay(stack, collector, TEXTURE_DYED_NOT, light, overlay, -1);
						}
					} else {
						float[] color = RatColorUtil.getDyeRgb(DyeColor.byId(rat.getDyeColor()));
						this.submitOverlay(stack, collector, TEXTURE_DYED, light, overlay, ARGB.colorFromFloat(1.0F, color[0], color[1], color[2]));
					}
				}
				...
				RatUpgradeUtils.forEachUpgrade(rat, item -> item instanceof ChangesOverlayUpgrade, (upgrade, slot) -> {
					if (rat.isSlotVisible(slot)) {
						...
						if (overlayType != null) {
							this.getParentModel().setupAnim(state);
							this.submitOverlay(stack, collector, overlayType, light, OverlayTexture.NO_OVERLAY, -1);
						}
					}
				});

				if (rat.hasToga()) {
					this.submitOverlay(stack, collector, TOGA_TEX, light, OverlayTexture.NO_OVERLAY, -1);
				}
			}
		}
	}

	private void submitOverlay(PoseStack stack, SubmitNodeCollector collector, RenderType type, int light, int overlay, int color) {
		collector.submitCustomGeometry(stack, type, (pose, consumer) ->
				this.getParentModel().renderCitadelToBuffer(stack, consumer, light, overlay, color));
	}
}
